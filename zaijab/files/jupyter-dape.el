...
decode-then-re-encode round-trip issues with booleans."
  (let ((content-part (plist-get msg :content)))
    (message "[jdape] message-content-json: content-part type=%S car=%S"
    ...
    (if (and (listp content-part)
        ...
        (progn
          (message "[jdape] message-content-json: using cached raw JSON, length=%d"
      ...
      (let* ((decoded (jupyter-message-content msg))
        ...
        (message "[jdape] message-content-json: re-encoded from plist, length=%d"
                 (length encoded))
        encoded))))

(defun jupyter-dape--send-to-dape (json-string)
  "Send JSON-STRING to DAPE via the TCP bridge."
  (message "[jdape] send-to-dape: client-alive=%S json-length=%d json-prefix=%.300s"
  ...
  (when (and jupyter-dape--client-process
    ...
    (let ((encoded (encode-coding-string json-string 'utf-8 t)))
      ...
      (message "[jdape] send-to-dape: sent %d bytes" (length encoded)))))
...
(defun jupyter-dape--route-debug-to-control (orig type)
  ...
  (let ((result (if (equal type "debug_request") "control" (funcall orig type))))
    (when (equal type "debug_request")
      (message "[jdape] route: type=%S -> channel=%S" type result))
    result))
  ...
  (let ((msg-type (jupyter-message-type msg)))
    (cond
     ((equal msg-type "debug_reply")
      (message "[jdape] intercept: debug_reply on channel=%S bridge-active=%S"
     ...
     ((equal msg-type "debug_event")
      ...
      (message "[jdape] intercept: swallowing debug_event (handled by subscriber)")
      nil)
...
(defun jupyter-dape--install-event-subscriber (client)
  ...
  (message "[jdape] install-event-subscriber: installing on client=%S gen=%d"
           client jupyter-dape--session-gen)
        ...
        (jupyter-subscriber
          (lambda (msg)
            (when (and jupyter-dape--bridge-active
              ...
              (message "[jdape] event-subscriber: forwarding debug_event gen=%d" my-gen)
              (jupyter-dape--send-to-dape
               (jupyter-dape--message-content-json msg)))))))))
...
Arrays become vectors (via :array-type \\='array)."
  (let ((result (json-parse-string json-string
                                   ...
                                   ;; 'vector is NOT valid.
                                   :array-type 'array)))
    (message "[jdape] parse-dap-json: parsed %d chars -> plist keys=%S"
             (length json-string)
...
(defun jupyter-dape--debug-send-sync (client command &optional arguments)
  ...
  (message "[jdape] send-sync: command=%S arguments=%S" command arguments)
    ...
    (let ((jupyter-inhibit-handlers t))
      (jupyter-run-with-client client
        (jupyter-mlet*
          ...
          (message "[jdape] send-sync: req id=%S type=%S"
...
instead of forwarding (the kernel rejects duplicate initialize)."
  (message "[jdape] forward-to-kernel: json-length=%d json-prefix=%.100s"
  ...
  (condition-case err
      (let* ((dap-plist (jupyter-dape--parse-dap-json json-string))
        ...
        (message "[jdape] forward-to-kernel: command=%S type=%S caps-cached=%S"
                 command (type-of command) (not (null jupyter-dape--init-capabilities)))
        (if (member command '("disconnect" "terminate"))
            ;; Do not forward disconnect/terminate to the kernel — ipykernel
            ;; cannot restart its debugpy session after receiving either one,
            ;; so we keep debugpy alive for reuse.  Send a synthetic success
            ;; response so DAPE does not hang, then mark the bridge inactive.
            ;; DAPE will close the TCP connection on its own after receiving
            ;; the response, which triggers the sentinel for cleanup.
            ;;
            ;; For terminate specifically, also interrupt the kernel so any
            ;; execution paused at a breakpoint is unblocked.
            (progn
              (message "[jdape] forward-to-kernel: intercepting %s, synthetic response" command)
              (when (equal command "terminate")
                (message "[jdape] forward-to-kernel: interrupting kernel to unblock paused execution")
                (ignore-errors
          ...
          (if (and (equal command "initialize")
            ...
            (let* ((json-false :json-false)
                   ...
                   (json-str (json-encode response)))
              (message "[jdape] forward-to-kernel: synthesizing initialize response from cache")
              (jupyter-dape--send-to-dape json-str)
              ...
              (let* ((evt `(:seq ,(jupyter-dape--next-seq)
                ...
                (message "[jdape] forward-to-kernel: sending synthetic initialized event")
          ...
          (let ((content-list (append dap-plist nil))
            ...
            (message "[jdape] forward-to-kernel: client=%S client-connected=%S"
                     (type-of client)
                     (and client (jupyter-connected-p client)))
            (jupyter-run-with-client client
              (jupyter-mlet*
                  ((req (apply #'jupyter-request "debug_request" content-list)))
                (message "[jdape] forward-to-kernel: req created id=%S type=%S"
                         (jupyter-request-id req)
                         (jupyter-request-type req))
                (jupyter-sent (jupyter-return req))))))))
    (error
...
(defun jupyter-dape--bridge-sentinel (proc event)
  ...
  (message "[jdape] bridge-sentinel: event=%S alive=%S" event (process-live-p proc))
...
(defun jupyter-dape--start-bridge ()
  ...
  (setq jupyter-dape--server-process
        (make-network-process
         ...
         :log (lambda (_server client _message)
                (message "[jdape] bridge-log: new client connection %S" client)
  ...
  (let ((port (process-contact jupyter-dape--server-process :service)))
    (message "[jdape] start-bridge: listening on port %d" port)
    port))

(defun jupyter-dape--stop-bridge ()
  "Tear down the TCP bridge."
  (message "[jdape] stop-bridge: tearing down")
  (setq jupyter-dape--bridge-active nil)
  (when (and jupyter-dape--client-process
             (process-live-p jupyter-dape--client-process))
...
(defun jupyter-dape--wrap-eval-string (orig str &optional insert beg end)
  ...
  (let* ((live-conns (and (fboundp 'dape--live-connections)
    ...
    (message "[jdape] wrap-eval-string: live-conns=%S has-client=%S lang=%S filepath=%S"
             (not (null live-conns)) (not (null has-client)) lang filepath)
    (if filepath
        (progn
          (message "[jdape] wrap-eval-string: wrapping for %s" filepath)
...
(defun jupyter-dape--ensure (config)
  ...
  (let* ((has-client (bound-and-true-p jupyter-current-client))
         (lang (and has-client
                    (ignore-errors
                      (jupyter-kernel-language jupyter-current-client)))))
    (message "[jdape] ensure: has-client=%S lang=%S" has-client lang)
...
Return CONFIG with the bridge port."
  (let ((client jupyter-current-client))
    (message "[jdape] configure: client=%S" (type-of client))
    ...
    (let* ((info (jupyter-dape--debug-send-sync client "debugInfo"))
           ...
           (started (and body (eq (plist-get body :isStarted) t))))
      (message "[jdape] configure: debugInfo body=%S started=%S" body started)
      (unless started
        (message "[jdape] configure: sending initialize to start debugpy")
        (let ((init-reply (jupyter-dape--debug-send-sync
                           ...
                           (list :adapterID "jupyter-dape"
                                 ...
                                 :columnsStartAt1 t
                                 :pathFormat "path"))))
          (message "[jdape] configure: initialize reply=%S"
                   (and init-reply
          ...
          (when init-reply
            ...
            (message "[jdape] configure: cached capabilities=%S"
        ...
        (let* ((info2 (jupyter-dape--debug-send-sync client "debugInfo"))
          ...
          (message "[jdape] configure: post-init debugInfo body=%S started=%S"
                   body2 started2)
          (unless started2
            (user-error
    ...
    (let ((port (jupyter-dape--start-bridge)))
      (message "[jdape] configure: bridge started on port %d" port)
      (setq jupyter-dape--bridge-active t)
      (jupyter-dape--install-event-subscriber client)
      (message "[jdape] configure: returning config with port=%d" port)
...
(defun jupyter-dape--register-config ()
  ...
  (message "[jdape] register-config: jupyter-dape registered"))
...
(define-minor-mode jupyter-dape-mode
  ...
  (cond
   (jupyter-dape-mode
    ...
    (message "[jdape] mode: enabled, advice installed"))
   (t
    (message "[jdape] mode: disabling")
    (jupyter-dape--stop-bridge)
