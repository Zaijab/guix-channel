    ...
    (source
     (origin
       ...
       (uri (git-reference
             ...
             (commit "1ef59f0aa02e3cff40bae68b756a29bc2001739e"))) ; new - cd2269ddb64bda7203acf2ee2e26188237a578ea
       ;; (file-name (git-file-name name version))
       (sha256
        (base32 "0p3afbs2j3m9sm9iiali3lqy202bwpy8k010d2wy7870cx7lbxch"))))
    (build-system emacs-build-system)
    (arguments
     (list
      #:tests? #f
  ...
  (package
    ...
    (source
     (origin
       (method git-fetch)
       (uri (git-reference
             (url "https://github.com/skissue/meow-tree-sitter.git")
             (commit "b05d54a5e19f8ba0258b0bbf0ffcd4bc7bad112c")))
       (file-name (git-file-name name version))
       (sha256
        (base32 "05v3gi29rnl47fnq1l3yrarsdyrfir5bx42rh2kj93f180jjp3z2"))))
    (build-system emacs-build-system)
    ...
    (description
     "`meow-tree-sitter is a package that integrates the treesit library in Emacs 29+
🌳 with Meow’s motions 🐱.  Lots of functionality is ported from
`evil-textobj-tree-sitter'.  To get started, call
`meow-tree-sitter-register-defaults to add the default keybinds to Meow's
\"thing\" registry.  They will now be accessible through \\\\[meow-bounds-of-thing]
and \\\\[meow-inner-of-thing], as well as \\\\[meow-beginning-of-thing] and
\\\\[meow-end-of-thing].  Evaluate (customize-group meow-tree-sitter) to see
customization options.  Options are also detailed in the README.org file
included in the project repository.  To register custom keybinds, call
`meow-tree-sitter-register-thing with a key and type (or list of types).  To
register custom queries, pass the optional third argument.  Persistent custom
queries can be registered in `meow-tree-sitter-extra-queries'.  See the
README.org file for details.  Queries in the `queries/ subdirectory were taken
